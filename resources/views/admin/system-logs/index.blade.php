<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto pb-20 space-y-6"
         x-data="{
             selectedIds: [],
             get allSelected() { return this.selectedIds.length === {{ count($allIds) }} && {{ count($allIds) }} > 0; },
             toggleAll() {
        ...
        <div class="flex flex-wrap gap-2">
            @foreach ($tabs as $key => $tab)
                ...
                <a href="{{ route('admin.system-logs.index', array_filter(['type' => $key, 'search' => $search ?: null, 'unread' => $unreadOnly ? null : '0'])) }}"
        ...
        <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
            ...
            <form action="{{ route('admin.system-logs.index') }}" method="GET" class="flex flex-1 gap-2">
                ...
                @if ($search)
                    <a href="{{ route('admin.system-logs.index', array_filter(['type' => $typeFilter !== 'all' ? $typeFilter : null, 'unread' => $unreadOnly ? null : '0'])) }}"
            ...
            @php
                $toggleParams = array_filter([
                    ...
                    'search' => $search ?: null,
                    'unread' => $unreadOnly ? '0' : '1',
                ]);
            @endphp
            <a href="{{ route('admin.system-logs.index', $toggleParams) }}"
               class="flex items-center gap-2 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all
                      {{ $unreadOnly ? 'bg-amber-50 text-amber-600 border border-amber-100' : 'bg-white text-slate-400 border border-slate-100 hover:text-slate-600' }}">
                <div class="w-1.5 h-1.5 rounded-full {{ $unreadOnly ? 'bg-amber-400' : 'bg-slate-300' }}"></div>
                {{ $unreadOnly ? 'Unread only' : 'All events' }}
            </a>
        ...
        @if ($logs->isEmpty())
            <div class="bg-white rounded-[3rem] border border-slate-100 shadow-xl p-20 text-center">
                ...
                @if ($unreadOnly)
                    ...
                    <a href="{{ route('admin.system-logs.index', array_filter(['type' => $typeFilter !== 'all' ? $typeFilter : null, 'search' => $search ?: null, 'unread' => '0'])) }}"
